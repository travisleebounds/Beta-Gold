    ...
    try:
        ...
        if related:
            ...
            with tab_programs:
                ...
                st.dataframe(pd.DataFrame(prog_list), width='stretch', hide_index=True)
            ...
            st.dataframe(pd.DataFrame(grant_table), width='stretch', hide_index=True, height=300)
        else:
...
# Load district boundaries (real GeoJSON from Census Bureau)
DISTRICT_BOUNDARIES = {}
if os.path.exists('il_congressional_boundaries.json'):
    with open('il_congressional_boundaries.json', 'r') as f:
        _raw_boundaries = json.load(f)
    # Map IL-01 format keys to match DISTRICTS dict
    for _bkey, _bval in _raw_boundaries.items():
        # Convert IL-01 to IL-01 (already matches), but also handle IL-1 etc.
        _dnum = int(_bkey.split('-')[1])
        _mapped_key = f"IL-{_dnum:02d}"
        DISTRICT_BOUNDARIES[_mapped_key] = _bval
    print(f"✅ Loaded {len(DISTRICT_BOUNDARIES)} real congressional boundaries")
elif os.path.isdir('data/boundaries'):
    # Load individual GeoJSON files from the boundaries directory
    for _i in range(1, 18):
        _path = f"data/boundaries/IL-{_i:02d}.geojson"
        if os.path.exists(_path):
            with open(_path, 'r') as f:
                DISTRICT_BOUNDARIES[f"IL-{_i:02d}"] = json.load(f)
    if DISTRICT_BOUNDARIES:
        print(f"✅ Loaded {len(DISTRICT_BOUNDARIES)} boundaries from data/boundaries/")
else:
    print("⚠️ No boundary files found. Run: python fetch_boundaries_v2.py")
...
def geojson_to_folium_coords(geometry):
    """
    Convert a GeoJSON geometry (Polygon or MultiPolygon) to a list of 
    folium-compatible coordinate rings: [ [[lat, lon], ...], ... ]
    Returns a list of rings (each ring is a list of [lat, lon] pairs).
    """
    if geometry is None:
        return []
    ...
    rings = []
    if geo_type == 'Polygon':
        # coords = [ [ [lon, lat], ... ] ]  (outer ring + optional holes)
        for ring in coords:
...
def add_district_boundary_to_map(m, district_id, info, boundary_data=None, fill_opacity=0.15, weight=2):
    """Add a district boundary (real or rectangle fallback) to a folium map."""
    color = '#4A90E2' if info['party'] == 'D' else '#E24A4A'
    popup_text = f"<b>{district_id}: {info['rep']} ({info['party']})</b><br>{info['area']}"
    
    if boundary_data and 'geometry' in boundary_data:
        rings = geojson_to_folium_coords(boundary_data['geometry'])
        for ring in rings:
            folium.Polygon(
                locations=ring,
                color=color,
        ...
        # Fallback: use the rectangle boundaries from DISTRICTS dict
...
if view == "🗺️ Statewide Map":
    ...
    for district_id, info in DISTRICTS.items():
        ...
        boundary_data = DISTRICT_BOUNDARIES.get(district_id)
        add_district_boundary_to_map(m, district_id, info, boundary_data, fill_opacity=0.15, weight=2)
    ...
    st.dataframe(pd.DataFrame(summary_data), width='stretch', hide_index=True, height=630)
...
# ==================== DISTRICT VIEW ====================
    ...
    if st.session_state.selected_district:
        ...
        # District map
        boundary_data = DISTRICT_BOUNDARIES.get(district)
        if boundary_data and 'geometry' in boundary_data:
            ...
            if geo_type == 'Polygon':
                ...
                for polygon in geo['coordinates']:
                    for lon, lat in polygon[0]:
                        all_lons.append(lon)
                        all_lats.append(lat)
            center_lat = sum(all_lats) / len(all_lats) if all_lats else info['lat']
            center_lon = sum(all_lons) / len(all_lons) if all_lons else info['lon']
        else:
            center_lat = info['lat']
            center_lon = info['lon']
        ...
        # Add boundary (real or rectangle)
        color = '#4A90E2' if info['party'] == 'D' else '#E24A4A'
        popup_text = f"<b>{district} Boundary</b>"
        ...
        st_folium(dm, width=1400, height=500, returned_objects=[])
...
# ==================== STATE LEGISLATORS ====================
    ...
    if road_data:
        ...
        if items:
            ...
            st.dataframe(pd.DataFrame(table_data), width='stretch', hide_index=True, height=360)
        else:
    ...
    if geo_tab == "⭐ US Senators (Statewide)":
        ...
        if road_data:
            ...
            if items:
                ...
                st.dataframe(pd.DataFrame(table_data), width='stretch', hide_index=True, height=500)
            else:
...
# ==================== AI ANALYSIS ====================
    ...
    with tab3:
        ...
        try:
            ...
            st.dataframe(pd.DataFrame(district_data), width='stretch', hide_index=True)
...
# ==================== IL GENERAL ASSEMBLY ====================
    ...
    try:
        ...
        tab1, tab2, tab3 = st.tabs(["🗺️ District Map", "📋 Bills", "👥 Legislators"])
        
        with tab1:
            st.subheader("Illinois Legislative District Map")

            # Load member data for popups
            try:
                with open('members.json', 'r') as f:
                    _map_members = json.load(f)
                _il_house = _map_members.get('il_house', {})
                _il_senate = _map_members.get('il_senate', {})
            except:
                _il_house = {}
                _il_senate = {}
            ...
            map_layer = st.radio(
                "Select map layer:",
                ["🏛️ Congressional Districts (17)", "🏠 IL House Districts (118)", "🏛️ IL Senate Districts (59)", "📊 All Layers"],
                horizontal=True,
                key="ilga_map_layer"
            )
            ...
            if map_layer.startswith("🏛️ Congressional") or map_layer.startswith("📊"):
                for i in range(1, 18):
                    dist_key = f"IL-{i:02d}"
                    boundary = DISTRICT_BOUNDARIES.get(dist_key)
                    if boundary and 'geometry' in boundary:
                        info = DISTRICTS.get(dist_key, {})
                        rep_name = info.get('rep', 'Unknown')
                        party = info.get('party', '?')
                        area = info.get('area', '')
                        color = '#2166ac' if party == 'D' else '#b2182b'
                        
                        popup_html = f"<b>{dist_key}: {rep_name} ({party})</b><br>{area}<br><i>Congressional District</i>"
                        
                        rings = geojson_to_folium_coords(boundary['geometry'])
                        for ring in rings:
                            folium.Polygon(
                                locations=ring,
                                color=color,
                                fill=True,
                                fillColor=color,
                                fillOpacity=0.15 if map_layer.startswith("📊") else 0.25,
                                weight=3,
                                popup=folium.Popup(popup_html, max_width=250),
                                tooltip=f"{dist_key}: {rep_name}"
                            ).add_to(m)
                        boundaries_loaded += 1

            # IL House districts
            if map_layer.startswith("🏠") or map_layer.startswith("📊"):
                for i in range(1, 119):
                    dist_key = f"IL-H-{i:03d}"
                    boundary = load_boundary(dist_key)
                    if boundary and 'geometry' in boundary:
                        member = _il_house.get(dist_key, {})
                        name = member.get('name', f'District {i}')
                        party = member.get('party', '?')
                        area = member.get('area', '')
                        color = '#4393c3' if party == 'D' else '#d6604d' if party == 'R' else '#999999'
                        
                        popup_html = f"<b>{dist_key}: {name}</b><br>{'🔵 D' if party == 'D' else '🔴 R' if party == 'R' else '⚪ ?'}<br>{area}<br><i>IL House District</i>"
                        ...
                        for ring in rings:
                            folium.Polygon(
                                ...
                                fill=True,
                                fillColor=color,
                                fillOpacity=0.12 if map_layer.startswith("📊") else 0.2,
                                weight=1.5,
                                popup=folium.Popup(popup_html, max_width=250),
                                tooltip=f"{dist_key}: {name}"
                            ).add_to(m)
                        boundaries_loaded += 1

            # IL Senate districts
            if map_layer.startswith("🏛️ IL Senate") or map_layer.startswith("📊"):
                for i in range(1, 60):
                    dist_key = f"IL-S-{i:03d}"
                    boundary = load_boundary(dist_key)
                    if boundary and 'geometry' in boundary:
                        member = _il_senate.get(dist_key, {})
                        name = member.get('name', f'District {i}')
                        party = member.get('party', '?')
                        area = member.get('area', '')
                        color = '#2166ac' if party == 'D' else '#b2182b' if party == 'R' else '#999999'
                        
                        popup_html = f"<b>{dist_key}: {name}</b><br>{'🔵 D' if party == 'D' else '🔴 R' if party == 'R' else '⚪ ?'}<br>{area}<br><i>IL Senate District</i>"
            ...
            if boundaries_loaded > 0:
                st.caption(f"Showing {boundaries_loaded} districts. Click a district for details.")
                st_folium(m, width=1400, height=650, returned_objects=[])
            else:
                st.warning("⚠️ No boundary files found. Run: `python fetch_boundaries_v2.py`")
            
            # Legend
            st.markdown("""
            **Legend:** 🔵 Democrat · 🔴 Republican · Colors vary by layer.  
            Congressional = bold outlines · House = thin outlines · Senate = medium outlines
            """)
        ...
        with tab3:
            ...
            try:
                with open('members.json', 'r') as f:
                    _tab_members = json.load(f)
                il_house = _tab_members.get('il_house', {})
                il_senate = _tab_members.get('il_senate', {})
            except:
                il_house = {}
                il_senate = {}
            ...
            # Search box
            search_query = st.text_input("🔍 Search by name, district, or area:", key="ilga_search", placeholder="e.g. Colvin, District 3, Chicago...")
            
            chamber_filter = st.radio("Chamber:", ["Both", "House", "Senate"], horizontal=True, key="ilga_chamber_filter")

            # Build combined member list for searching
            all_members = []
            if chamber_filter in ("Both", "House"):
                for key, member in il_house.items():
                    all_members.append({
                        'key': key,
                        ...
                        'area': member.get('area', ''),
                        'chamber': 'House',
                        'nav_chamber': 'IL House (118)',
                    })
            if chamber_filter in ("Both", "Senate"):
                for key, member in il_senate.items():
                    all_members.append({
                        'key': key,
                        'name': member.get('name', 'Unknown'),
                        'party': member.get('party', '?'),
            ...
            if search_query:
                ...
                all_members = [m for m in all_members if (
                    ...
                    q in m['area'].lower() or
                    q in m['key'].lower() or
                    q in str(m['district']) or
                    q in m['chamber'].lower()
                )]
            
            all_members.sort(key=lambda x: (x['chamber'], x['district']))

            if all_members:
                st.markdown(f"**Showing {len(all_members)} members** — click to open profile")
                
                # Display as a searchable table with buttons
                for m in all_members:
                    party_icon = '🔵' if m['party'] == 'D' else '🔴' if m['party'] == 'R' else '⚪'
                    label = f"{party_icon} {m['key']} — {m['name']} ({m['chamber']}) — {m['area']}"
                    
                    def make_nav(nav_chamber=m['nav_chamber'], dist=m['district']):
                        def handler():
                            st.session_state['_pending_nav'] = '👥 State Legislators'
                            st.session_state['_pending_chamber'] = nav_chamber
